...
const validateDate = (date: Date) => {
  if (date.getTime() > Date.now()) {
    throw new Error(
      `You cannot delete documents that were created in the future!`
    )
  }
...
const dateToObjectId = (date: Date) => {
...
const checkIfCollectionExists = async ({
  ...
  if (!collectionExists) {
    throw new Error(
      `Collection '${collectionName}' does not exist or has 0 documents!`
    )
  }
}
...
const bulkDelete = async ({
  ...
  documentDeleteBatchSize = 300,
  sleepDurationBetweenDeletesSize = 200,
}: {
  mongoDbUri: string
  dbName: string
  collectionName: string
  deleteBeforeThisDate: Date
  documentDeleteBatchSize?: number
  sleepDurationBetweenDeletesSize?: number
}) => {
  if (!mongoDbUri) {
  ...
  if (documentDeleteBatchSize <= 0) {
    throw new Error(
      `documentDeleteBatchSize must be greater than 0! (documentDeleteBatchSize: ${documentDeleteBatchSize})`
    )
  }
  if (sleepDurationBetweenDeletesSize <= 0) {
    throw new Error(
      `sleepDurationBetweenDeletesSize must be greater than 0! (sleepDurationBetweenDeletesSize: ${sleepDurationBetweenDeletesSize})`
    )
  }
  ...
  const toBeRemainingDocumentCount =
    totalCollectionDocumentCount - toBeDeletedDocumentCount
...
Created before ` +
  ...
  const progressBar = new cliProgress.SingleBar(
    {
      format:
        'Progress | {bar} | {percentage}% | {value}/{total} Documents | Elapsed: {duration_formatted} | Time remaining: {eta_formatted}',
    },
  ...
  do {
    ...
    progressBar.update(deletedDocumentCount, {
      ...
      eta_formatted: formatEta(
        startTime,
        deletedDocumentCount,
        toBeDeletedDocumentCount
      ),
    })
